...
public class MenuActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        instance = Quiz.getInstance(getResources().getStringArray(R.array.profileIcon));

        answer = findViewById(R.id.edtAnswer);
        question = findViewById(R.id.txtQuest);
        confirm = findViewById(R.id.btnConfirm);

        question.setText(instance.requestQuest("Arch"));

        confirm.setOnClickListener(l -> {
            if (answer.getText().toString().trim()
                    .equals(instance.requestAnswer("Arch", question.getText().toString()))){
